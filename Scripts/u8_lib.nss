...
// Set flag for player(s), for has found portal, using the portal's tag as the flag key.
void U8SetFoundRecalPortal(object oPC, string sPortalTag)
{
	SetLocalInt(oPC, sPortalTag, 1);
}

// Get flag for player(s), for has found portal, using the portal's tag as the flag key.
int U8HasFoundRecalPortal(object oPC, string sPortalTag)
{
	return GetLocalInt(oPC, sPortalTag);
}
